              ...
              sh '''
                ...
                curl -fsS ${MANAGER_URL}/login >/dev/null || {
                  ...
                  exit 1
                        ...
                        DISPLAY=:99 timeout 10m mvn -B -ntp -Dtest="com.revature.api.*Test,com.revature.DAOs.*Test,com.revature.models.*Test,com.revature.utils.*Test,com.revature.TableFormatterTest" -Demployee.baseUrl=${EMPLOYEE_URL} -Dmanager.baseUrl=${MANAGER_URL} clean test
              '''
            } else {
              bat '''
                ...
                docker rm -f employee-app manager-app 2>$null
                    ...
                    powershell -NoProfile -Command "$job = Start-Job { & mvn -B -ntp '-Dtest=com.revature.api.*Test,com.revature.DAOs.*Test,com.revature.models.*Test,com.revature.utils.*Test,com.revature.TableFormatterTest' (\"-Demployee.baseUrl=$env:EMPLOYEE_URL\") (\"-Dmanager.baseUrl=$env:MANAGER_URL\") clean test }; if (-not (Wait-Job $job -Timeout 600)) { Stop-Job $job; throw 'Maven tests timed out after 10 minutes' }; Receive-Job $job; if ((Get-Job $job).State -ne 'Completed') { exit 1 }"
              '''
            }
          }
        }
